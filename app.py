...
def scrape_place_reviews(place_id):
    # Streamlit Cloud 환경을 위한 Headless 크롬 옵션 설정
    options = Options()
    ...
    # 1) 가상 브라우저 실행 및 플레이스 방문자 리뷰 탭 접속
    url = f"https://m.place.naver.com/restaurant/{place_id}/review/visitor"
    
    try:
        driver = webdriver.Chrome(options=options)
        driver.get(url)
        # 동적 페이지(React)가 렌더링될 때까지 충분히 대기 (매우 중요)
        time.sleep(4) 
        
        # 2) 스크롤을 살짝 내려서 추가 리뷰 로딩 유도 (최근 리뷰 파악용)
        driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
        time.sleep(2)
        
        # 3) '사장님 답글' 텍스트가 포함된 요소 개수 카운팅
        # 네이버는 클래스명이 수시로 바뀌므로 텍스트 기반 탐색이 가장 안전합니다.
        replies = driver.find_elements(By.XPATH, "//*[contains(text(), '사장님 답글')]")
        reply_count = len(replies)
        
        # 4) 화면에 로딩된 전체 리뷰 박스 카운팅 (대략적인 최신 리뷰 수)
        review_boxes = driver.find_elements(By.CSS_SELECTOR, "li.owAeM") # 이 클래스명은 네이버 업데이트에 따라 바뀔 수 있음
        total_visible_reviews = len(review_boxes) if len(review_boxes) > 0 else 50 # 예외 처리
...
if submit_button:
    if not naver_client_id or not naver_client_secret:
        ...
        with st.spinner('실시간으로 네이버 데이터를 스크래핑하고 API를 호출하고 있습니다. (약 10초 소요)...'):
            ...
            st.markdown("### 💬 1. 플레이스 고객 소통 지수 (데이터 추출)")
            ...
            st.markdown("### 📝 2. 블로그 리뷰노트 누적 및 노출 (API 추출)")
