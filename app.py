...
st.markdown("자동 API 추출과 핵심 수기 데이터를 결합하여 실질적인 홍보 효과를 분석합니다.")
...
with st.form("report_form"):
    st.subheader("📌 1. 타겟 키워드 및 순위 상승 지표")
    ...
    with col3:
        # 전월 순위 대신 '상승폭'을 바로 입력받습니다.
        rank_increase = st.number_input("이번 달 순위 상승폭 (유지 시 0 입력)", min_value=0, value=5)

    st.divider()

    st.subheader("📝 2. 블로그 홍보 성과 및 핵심 링크")
    blog_link = st.text_input("이번 달 대표 우수 블로그 링크 (보고서에 노출됩니다)", placeholder="https://blog.naver.com/...")
...
if submit_button:
    if not store_name or not promo_keyword:
        ...
        with st.spinner('API로 현재 순위를 추출하고 홍보 효과를 분석 중입니다...'):
            ...
            # 순위 증감 텍스트 생성 (API가 찾은 현재 순위 + 대표님이 입력한 상승폭)
            rank_text = ""
            if current_rank == 0:
            ...
            if blog_link:
                st.markdown(f"> **핵심 홍보 블로그 링크:** {blog_link}")
